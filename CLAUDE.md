...
- ロール識別色（`@RoleBadge` の admin=紫 / editor=青 / viewer=灰）は**色相**を
  カテゴリ色として固定（ブランドテーマ Vercel/Notion では変えない例外）。ただし
  明暗（ライト/ダーク）には追従させる: ライトは淡色地＋濃文字、ダークは半透明地＋
  淡文字を `dark:` 変種で指定する（生パレットだがこのバッジに限る例外）。
  `dark:` を `.dark` クラス連動にするため `input.css` 冒頭に
  `@custom-variant dark (&:where(.dark, .dark *));` を置いている。
...
- 環境変数を追加・削除したら `.env.example` も更新する（意味・デフォルト値・必須/任意を記載）。
  - `.env.example` が「変数の意味」の正。本番の値は別リポ nz-vps-ops の `apps/<ホスト名>.yml`（`app_env_vars`）で設定する
  - コードの `os.Getenv` との食い違い（記載漏れ・残骸）は `make check-env-docs`（`make check` に含む）で検出される
- DB スキーマの変更は **`db/README.md` の手順に従う**。`db/migrations/`（goose、実 DB の正）と
  `db/schema*.sql`（sqlc 用の完成形参照）は手動同期が必須で、自動検証はない。
  片方だけの変更は事故になる（migration のみ → sqlc が新カラムを知らない /
  schema のみ → 実行時 no such column）。カラム変更・削除はテーブル再作成の儀式が必要
  （手順と FK の罠は `db/README.md` 参照）
