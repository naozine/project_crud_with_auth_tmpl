...
- **ボタン・カード・フォーム・バッジ・ダイアログは既存コンポーネントを使う**。
  ...
  - `web/components/ui_button.templ`: `@PrimaryButton` `@PrimaryActionButton`
    `@SecondaryButton` `@DangerButton` `@DangerTextButton` `@PrimaryLink`
    `@GhostIconButton`（控えめなアイコン操作）ほか
...
- お手本（主）: `web/components/access_logs.templ`（`/admin/access-logs`）—
  **ヘッダ固定テーブルの一覧**の基準実装。一覧系の新規画面はこれに合わせる。
  - デスクトップは `ui_table` のテーブル（ページは動かずテーブル内だけスクロール、
    見出し行は sticky）、モバイルはカードへ自動で切り替えるレスポンシブ構成。
    高さは Shell の fixed な `main` から flex で受ける（組み方は `ui_table.templ`
    冒頭のコメント参照。マジックナンバー禁止）。
  - 更新などの補助操作は専用の行を作らず、見出し行右端の `@ThTools` +
    `@GhostIconButton`（モバイルはカード上に右寄せ）に置く。
  - 一覧の SSE 更新は **コンテナ（`#access-logs-list`）ごと inner 置換**する。テーブルと
    カードの2系統を同期させるため、行単位 outer 置換にはしない
    （`internal/handlers/access_log.go` の `TableSSE` 参照）。reload・PRG はしない。
- お手本（CRUD 付き一覧）: `web/components/admin_users_list.templ` — 同じテーブル方式に
  ダイアログ編集・作成・削除が付く場合の基準実装（`internal/handlers/sse_admin.go` の
  `patchUserList` でコンテナ inner 置換）。
- お手本（副）: `web/components/project_list.templ` — カード一覧 + ダイアログ作成の基準実装。
  URL を持たない小さな項目をカードで並べる場合はこちら。
