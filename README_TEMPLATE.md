...
- [ ] **フォルダ名**: `Makefile` は `notdir $(CURDIR)` から `PROJECT_NAME` を生成し、Docker イメージ名・Cookie 名・fly.io app 名・Litestream バケット名に流用します（`_` は `-` に自動変換）。
...
- [ ] **WebAuthn RP Name**:
  ...
  - 環境変数: `docker-compose.yaml` / `docker-compose.dev.yaml` の `WEBAUTHN_RP_NAME`
...
- [ ] `make fly-setup`、シークレット設定、`make fly-deploy`

##### VPS (Docker + Caddy)
- [ ] `cp deploy.config.example deploy.config`
- [ ] `cp .env.production.example .env.production`
- [ ] `make caddy-setup`、`make docker-deploy`
